...
void Launcher::boot_anim()
{
    ...
    // Show boot image
    GetHAL().display.pushImage(0, 0, 240, 135, image_data_boot);
    GetHAL().display.setFont(&fonts::efontCN_16);
    GetHAL().display.setTextColor((uint32_t)0x1C1C1C);
    int textWidth = GetHAL().display.textWidth(FW_VERSION);
    // Under the corner label rather than along the bottom: the prompts
    // moved right to clear the mascot and now reach where this used to sit.
    GetHAL().display.drawString(FW_VERSION, 235 - textWidth, 18);
